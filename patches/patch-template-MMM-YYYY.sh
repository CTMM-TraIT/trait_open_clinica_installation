...
# -----------------------------------------------------------------------------------------------------------
# 
# Upgrade / patch script to deploy TraIT patches and upgrades of Tomcat.
# 
# Usage history:
# Feb 2016: TraIT OpenClinica patch and Tomcat upgrade to version 7.0.67.
#
# -----------------------------------------------------------------------------------------------------------
...
#
# Don't forget to remove this line when the debugging / developing this script is finished.
#
...
#
# For the SANDBOX environment we must also copy the setenv.sh config file 
# to avoid problems in the RuleDesigner. See also TraIT1602 118
#
echo "Exiting now. Test this SANDBOX specific step next release".
exit 14;
if [ $OC_ENVIRONMENT  = "SANDBOX" ]; then
  echo "Copying the setenv.sh file; only in the SANDBOX environment"
  cp -p ../tomcat/bin/setenv.sh ./bin
fi
